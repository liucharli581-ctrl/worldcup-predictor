...
async function main() {
  ...
  // Get all teams mapped by fifaCode - using raw query since Prisma might have version issues
  const teams = db.prepare("SELECT id, name, fifa_code as fifaCode FROM teams").all()
  const teamByCode = new Map(teams.map(t => [t.fifaCode, t]))

  // Clear existing H2H
  db.prepare("DELETE FROM head_to_head").run()

  const insert = db.prepare(
    "INSERT INTO head_to_head (id, home_team_id, away_team_id, home_team_name, away_team_name, match_date, competition, stage, home_score, away_score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  )

  const insertMany = db.transaction((rows) => {
    for (const [homeCode, awayCode, dateStr, competition, stage, homeScore, awayScore] of rows) {
      ...
      const id = `${homeCode}_${awayCode}_${dateStr.replace(/-/g, "")}`
      insert.run(id, home.id, away.id, home.name, away.name, dateStr, competition, stage, homeScore, awayScore)
    }
  })

  insertMany(HISTORICAL_MATCHES)
  console.log(`Seeded ${HISTORICAL_MATCHES.length} historical head-to-head records`)
}
